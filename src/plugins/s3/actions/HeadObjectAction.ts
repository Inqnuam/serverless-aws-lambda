...
export class HeadObjectAction extends S3LocalService {
  filePath: string;
  bucketPath: string;
  bucket: string;
  key: string;
  ...
  constructor(url: URL, headers: IncomingHttpHeaders) {
    ...
    this.bucket = bucket;
    this.key = key.join("/");
    this.bucketPath = path.join(S3LocalService.localStoragePath, bucket);
    this.filePath = path.join(this.bucketPath, ...key); // leave ...key as it is for Windows compatibility
